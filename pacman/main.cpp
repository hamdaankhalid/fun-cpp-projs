...
class Gameboard {
	...
	private:
		...
		std::pair<std::pair<int, int>, Direction> validateMove(std::pair<int, int>& currPos, Direction dir);
...
void Gameboard::draw(std::vector<std::unique_ptr<Input> >& updates) {
	...
	for (int i = 0; i < pidCounter; i++) {
		...
		std::cout << "moving PID: " << i << " TO " << pos.y << " , " << pos.x << std::endl;
		char repr;
		if (i == 0) {
			repr = PACMAN;
		} else {
...
int Gameboard::insertMovable() {
	...
	this->movables[this->pidCounter] = std::make_unique<Position>(Position{row, col});
	this->pidCounter++;
...
std::pair<std::pair<int, int>, Direction> Gameboard::validateMove(std::pair<int, int>& currPos, Direction dir) {
	switch (dir) {
		case UP:
			if (currPos.first > 0)
...
std::unique_ptr<Input> Ghost::getNextMove(Gameboard& gb) {
	while (true) {
		...
		// would making the move be a valid move on the board? if not give me a random diff dir
		Input ghostInput;
...
int main() {
	...
	std::cout << "2 ghosts added with ids " << ghost1Id << ", and " << ghost2Id << std::endl;
	...
	{
		for (int i = 0; i < 3; i++) {
			gameplayInstructionBuffer.push_back(
				ghosts[i].getNextMove(gb)
			);
		}
